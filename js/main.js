// THE MOON - Complete JavaScript File
// News data will be loaded dynamically
...
let newsData = {
    en: [
        { id: 0, title: "Welcome to THE MOON! Create an issue with 'news' label to add news", time: "Just now", category: "news", views: "0", comments: "0" }
    ],
    hi: []
};

// Try to load external news data
async function loadNewsData() {
    try {
        const response = await fetch('js/news-data.js');
...
const breakingNewsData = {
    en: ["🚨 Create issue with 'news' label to publish!", "🚨 Add your first news today!", "🚨 Share with friends!"],
    hi: ["🚨 'news' लेबल के साथ issue बनाएं!", "🚨 आज अपनी पहली खबर डालें!"]
};
...
function renderNews() {
    ...
    if (data.length === 0) {
        newsList.innerHTML = '<div class="empty-state"><i class="fas fa-newspaper"></i><p>No news yet. Create an issue with "news" label!</p></div>';
        return;
    }
...
function initPWA() {
    ...
    window.addEventListener('beforeinstallprompt', (e) => {
        ...
        document.getElementById('pwaInstallBtn').onclick = async () => { if (deferredPrompt) { deferredPrompt.prompt(); const { outcome } = await deferredPrompt.userChoice; if (outcome === 'accepted') { if (container) container.style.display = 'none'; } deferredPrompt = null; } };
        document.getElementById('pwaDismissBtn').onclick = () => { if (container) container.style.display = 'none'; };
    });
}

function initOfflineMode() {
    function update() { const indicator = document.getElementById('offlineIndicator'); if (indicator) indicator.style.display = !navigator.onLine ? 'block' : 'none'; }
